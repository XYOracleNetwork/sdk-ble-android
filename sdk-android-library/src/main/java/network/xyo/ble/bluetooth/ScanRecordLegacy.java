...
public final class ScanRecordLegacy {

    final private static String TAG = ScanRecordLegacy.class.getSimpleName();
     ...
     */
    @Nullable
    public static ScanRecordLegacy parseFromBytes(@Nullable byte[] scanRecord) {
        if (scanRecord == null) {
            return null;
        }
        ...
        return new ScanRecordLegacy(serviceUuids, manufacturerData, serviceData,
                advertiseFlag, txPowerLevel, localName, scanRecord);
    }

    // Parse service UUIDs.
    private static int parseServiceUuid(@NonNull byte[] scanRecord, int currentPos, int dataLength,
                                        int uuidLength, @NonNull List<ParcelUuid> serviceUuids) {
        while (dataLength > 0) {
     ...
     */
    @Nullable
    public byte[] getServiceData(@Nullable ParcelUuid serviceDataUuid) {
